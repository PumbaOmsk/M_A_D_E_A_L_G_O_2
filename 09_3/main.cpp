...
/// максимальное количество людей по направлению
static const long MAX_PEOPLE = 10000;
...
/// хранение одного объекта
struct Object {
    int x, y, p;
};

/// расстояние м/у объектами
int dist(const Object &from, const Object &to) {
    return abs(from.x - to.x) + abs(from.y - to.y) + 1;
}

/// заполнить граф эвакуации
void fillGraph(OGraph &g, const vector<Object> &office, const vector<Object> &bunker) {
    auto n = office.size();
    auto m = bunker.size();
    for (int i = 1; i <= n; ++i) {
        for (int j = 1; j <= m; ++j) {
            long w = dist(office[i - 1], bunker[j - 1]);
            g.addOr(i, n + j, MAX_PEOPLE, w, i * n + j);
        }
        g.addOr(0, i, office[i - 1].p, 0, n * m + i);
    }
    for (int j = 1; j <= m; ++j) {
        g.addOr(n + j, n + m + 1, bunker[j - 1].p, 0, n * m + n + j);
    }
}
...
int main() {
    int n, m;
    cin >> n >> m;
    vector<Object> office;
    for (int i = 0; i < n; ++i) {
        int x, y, p;
        cin >> x >> y >> p;
        office.push_back({x, y, p});
    }
    vector<Object> bunker;
    for (int i = 0; i < m; ++i) {
        int x, y, p;
        cin >> x >> y >> p;
        bunker.push_back({x, y, p});
    }
    int base_flow = 0;
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < m; ++j) {
            int p;
            cin >> p;
            base_flow += p * dist(office[i], bunker[j]);
        }
    }

    OGraph g(static_cast<size_t>(n + m + 2));
    fillGraph(g, office, bunker);
